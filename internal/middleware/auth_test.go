...
func TestJWTAuth_PasswordOnlyToken(t *testing.T) {
	secret := "test-secret"
	claims := jwt.MapClaims{"sub": "admin", "email": "", "exp": jwt.NewNumericDate(time.Now().Add(time.Hour))}
	...
	handler := mw.JWTAuth(secret)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if val, ok := r.Context().Value(mw.EmailKey).(string); ok {
			receivedUser = val
		}
		w.WriteHeader(http.StatusOK)
	}))
...
func TestJWTAuth_EmptyIdentityToken(t *testing.T) {
	secret := "test-secret"
	claims := jwt.MapClaims{"sub": "", "email": "", "exp": jwt.NewNumericDate(time.Now().Add(time.Hour))}
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, claims)
	s, err := token.SignedString([]byte(secret))
	assert.NoError(t, err)
