...
func (r *SeasonAuditRepository) DuplicateSeriesGroups() ([]DuplicateGroup, error) {
	...
	for _, c := range columns {
		...
		query := fmt.Sprintf(`
			...
			JOIN (
				...
				WHERE type = 'series' AND match_status = 'confirmed' AND %[1]s IS NOT NULL AND %[1]s != '' AND %[1]s != 0 AND %[1]s != '0'
				GROUP BY %[1]s HAVING COUNT(*) > 1
			) d ON t.%[1]s = d.%[1]s
			WHERE t.type = 'series' AND t.match_status = 'confirmed' AND t.%[1]s IS NOT NULL AND t.%[1]s != '' AND t.%[1]s != 0 AND t.%[1]s != '0'`, c.column)
	...
	slices.SortFunc(groups, func(a, b DuplicateGroup) int {
		if len(a.Titles) > 0 && len(b.Titles) > 0 {
			return cmp.Compare(a.Titles[0].ID, b.Titles[0].ID)
		}
		return cmp.Compare(a.SharedID, b.SharedID)
	})
