...
func TestSeasonAudit_DuplicateSeriesGroups_IgnoresEmptyAndZeroIDs(t *testing.T) {
	db := setupTestDB(t)
	repo := repository.NewSeasonAuditRepository(db)

	empty := ""
	_ = confirmedSeries(t, db, "Series With Empty IMDB 1", &empty, nil, nil)
	_ = confirmedSeries(t, db, "Series With Empty IMDB 2", &empty, nil, nil)

	var zeroTMDB int64 = 0
	_ = confirmedSeries(t, db, "Series With Zero TMDB 1", nil, &zeroTMDB, nil)
	_ = confirmedSeries(t, db, "Series With Zero TMDB 2", nil, &zeroTMDB, nil)

	var zeroTVDB int64 = 0
	_ = confirmedSeries(t, db, "Series With Zero TVDB 1", nil, nil, &zeroTVDB)
	_ = confirmedSeries(t, db, "Series With Zero TVDB 2", nil, nil, &zeroTVDB)

	groups, err := repo.DuplicateSeriesGroups()
	require.NoError(t, err)
	assert.Empty(t, groups, "empty strings and zero IDs must not create duplicate groups")
}
