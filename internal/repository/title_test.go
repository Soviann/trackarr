...
func TestTitleRepository_FindByExternalID(t *testing.T) {
	...
	anilist := int64(67890)
	tvdb := int64(54321)
	testutil.CreateTitle(t, db, &model.Title{Type: model.TitleTypeMovie, Year: 2024, Status: model.TitleStatusWatching, MatchStatus: model.MatchStatusConfirmed, IMDBID: &imdb, TMDBID: &tmdb, AniListID: &anilist, TVDBID: &tvdb}, []model.TitleName{{Name: "Test Movie", Language: "en", IsPrimary: true}})
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, err := repo.FindByExternalID(tc.imdbID, tc.tmdbID, tc.plexKey, tc.anilistID, tc.tvdbID, nil)
			if tc.wantErr {
				assert.Error(t, err)
				return
			}
...
func TestTitleRepository_FindByNameAndType(t *testing.T) {
	db := setupTestDB(t)
	repo := repository.NewTitleRepository(db)

	// Create a series with both English and French names
	testutil.CreateTitle(t, db, &model.Title{
		...
		{Name: "WildC.A.T.S: Covert Action Teams", Language: "en", IsPrimary: true},
		{Name: "Wild C.A.T.S: le commando galactique", Language: "fr", IsPrimary: false},
	})

	// Create a movie with same/similar name to test type discrimination
	testutil.CreateTitle(t, db, &model.Title{
		Type:        model.TitleTypeMovie,
		Year:        1985,
		...
		{Name: "Commando", Language: "en", IsPrimary: true},
		{Name: "Commando", Language: "fr", IsPrimary: false},
	})

	// Create a reboot series with different year
	testutil.CreateTitle(t, db, &model.Title{
		Type:        model.TitleTypeSeries,
		Year:        2024,
		Status:      model.TitleStatusWatching,
		...
		{Name: "The Flash", Language: "en", IsPrimary: true},
	})
	testutil.CreateTitle(t, db, &model.Title{
		Type:        model.TitleTypeSeries,
		Year:        1990,
	...
	t.Run("matches primary English name case-insensitively", func(t *testing.T) {
		got, err := repo.FindByNameAndType("wildc.a.t.s: covert action teams", model.TitleTypeSeries, 1994)
		require.NoError(t, err)
		assert.Equal(t, "WildC.A.T.S: Covert Action Teams", got.PrimaryName())
	})

	t.Run("matches secondary French alias", func(t *testing.T) {
		got, err := repo.FindByNameAndType("Wild C.A.T.S: le commando galactique", model.TitleTypeSeries, 1994)
		require.NoError(t, err)
		assert.Equal(t, "WildC.A.T.S: Covert Action Teams", got.PrimaryName())
	})

	t.Run("type must match", func(t *testing.T) {
		_, err := repo.FindByNameAndType("Commando", model.TitleTypeSeries, 1985)
		assert.ErrorIs(t, err, sql.ErrNoRows)

		got, err := repo.FindByNameAndType("Commando", model.TitleTypeMovie, 1985)
		require.NoError(t, err)
		assert.Equal(t, "Commando", got.PrimaryName())
	})

	t.Run("prioritizes exact year match", func(t *testing.T) {
		got1990, err := repo.FindByNameAndType("The Flash", model.TitleTypeSeries, 1990)
		require.NoError(t, err)
		assert.Equal(t, 1990, got1990.Year)

		got2024, err := repo.FindByNameAndType("The Flash", model.TitleTypeSeries, 2024)
		require.NoError(t, err)
		assert.Equal(t, 2024, got2024.Year)
	})

	t.Run("returns ErrNoRows on empty name or non-existing title", func(t *testing.T) {
		_, err := repo.FindByNameAndType("", model.TitleTypeSeries, 0)
		assert.ErrorIs(t, err, sql.ErrNoRows)

		_, err = repo.FindByNameAndType("Non Existent Title", model.TitleTypeSeries, 2020)
		assert.ErrorIs(t, err, sql.ErrNoRows)
	})
}
...
func TestTitleRepository_Merge_TransfersMissingExternalIDs(t *testing.T) {
	...
	require.NotNil(t, got.ExternalSourceID)
	assert.Equal(t, plexKey, *got.ExternalSourceID)
}
...
func TestTitleRepository_Merge_PreservesExistingExternalIDs(t *testing.T) {
	...
	require.NotNil(t, got.ExternalSourceID)
	assert.Equal(t, destExtKey, *got.ExternalSourceID)
	// NULL dest value must be filled from source.
	require.NotNil(t, got.TVDBID)
	assert.Equal(t, srcTvdb, *got.TVDBID)
}

func TestTitleRepository_Merge_TransfersMetadataAndArrIDs(t *testing.T) {
	db := setupTestDB(t)
	repo := repository.NewTitleRepository(db)
	...
	destID := testutil.CreateTitle(t, db, &model.Title{
		...
		ArrIgnored:  true,
	}, []model.TitleName{{Name: "Wild C.A.T.S", Language: "en", IsPrimary: true}})

	sonarrID := int64(1234)
	myRating := 9
	overview := "Wild C.A.T.S superhero series"
	sourceID := testutil.CreateTitle(t, db, &model.Title{
		Type:        model.TitleTypeSeries,
		Year:        1994,
		Status:      model.TitleStatusWatching,
		...
		MyRating:    &myRating,
		Overview:    &overview,
		ArrIgnored:  false, // in queue
	}, []model.TitleName{{Name: "Wild C.A.T.S: le commando galactique", Language: "fr", IsPrimary: true}})

	// Add a genre on source
	_, err := db.Exec(`INSERT INTO title_genres (title_id, genre) VALUES (?, 'Animation')`, sourceID)
	require.NoError(t, err)
	...
	got, err := repo.GetByID(destID)
	require.NoError(t, err)
	require.NotNil(t, got.SonarrID)
	assert.Equal(t, sonarrID, *got.SonarrID)
	require.NotNil(t, got.MyRating)
	assert.Equal(t, myRating, *got.MyRating)
	require.NotNil(t, got.Overview)
	assert.Equal(t, overview, *got.Overview)
	assert.False(t, got.ArrIgnored, "arr_ignored must be false if source was in queue")
	assert.Equal(t, model.MatchStatusConfirmed, got.MatchStatus, "match_status must upgrade to confirmed")
	assert.Contains(t, got.Genres, "Animation", "genre must be transferred")
}

func TestTitleRepository_Merge_CollidingSeasonPreservesWatchedStatusAndEvents(t *testing.T) {
	db := setupTestDB(t)
	repo := repository.NewTitleRepository(db)
	...
	destID := testutil.CreateTitle(t, db, &model.Title{
		...
		MatchStatus: model.MatchStatusConfirmed,
		Runtime:     &runtime,
	}, []model.TitleName{{Name: "Series Dest", Language: "en", IsPrimary: true}})
	destS1 := testutil.GetOrCreateSeason(t, db, destID, 1)
	// Dest ep 1 is unwatched
	testutil.SeedEpisode(t, db, destS1.ID, 1, "2020-01-01", false)
	...
	sourceID := testutil.CreateTitle(t, db, &model.Title{
		...
		MatchStatus: model.MatchStatusConfirmed,
		Runtime:     &runtime,
	}, []model.TitleName{{Name: "Series Source", Language: "en", IsPrimary: true}})
	srcS1 := testutil.GetOrCreateSeason(t, db, sourceID, 1)
	// Source ep 1 is watched
	srcEp1 := testutil.SeedEpisode(t, db, srcS1.ID, 1, "2020-01-01", true)
	// Source ep 2 is unwatched (new ep)
	testutil.SeedEpisode(t, db, srcS1.ID, 2, "2020-01-08", false)

	// Create a watch event pointing to source ep 1
	var weID int64
	err := db.QueryRow(`INSERT INTO watch_events (title_id, episode_id, source, created_at) VALUES (?, ?, 'plex', '2020-01-02 20:00:00') RETURNING id`, sourceID, srcEp1.ID).Scan(&weID)
	require.NoError(t, err)
	...
	ep1 := got.Seasons[0].Episodes[0]
	assert.Equal(t, 1, ep1.Episode)
	assert.True(t, ep1.Watched, "dest ep 1 must be updated to watched=true from source")

	ep2 := got.Seasons[0].Episodes[1]
	assert.Equal(t, 2, ep2.Episode)
	assert.False(t, ep2.Watched)

	// Verify watch event was re-parented to dest title and dest ep 1
	var weTitleID, weEpID int64
	err = db.QueryRow(`SELECT title_id, episode_id FROM watch_events WHERE id = ?`, weID).Scan(&weTitleID, &weEpID)
	require.NoError(t, err)
	assert.Equal(t, destID, weTitleID)
	assert.Equal(t, ep1.ID, weEpID, "watch_event must point to dest ep 1 ID")

	// Total watch minutes must equal runtime * 1 watched episode = 25
	assert.Equal(t, 25, got.TotalWatchMinutes)
}
...
func TestTitleWriter_Update_ConvertToMovie_PurgesSeasons(t *testing.T) {
	db := setupTestDB(t)
	repo := repository.NewTitleRepository(db)

	seriesStatus := model.SeriesStatusEnded
	titleID := testutil.CreateTitle(t, db, &model.Title{
		Type:         model.TitleTypeSeries,
		Status:       model.TitleStatusCompleted,
		MatchStatus:  model.MatchStatusConfirmed,
	...
	_ = testutil.GetOrCreateEpisode(t, db, seasonID, 1)
	...
	// Verify in DB that seasons and episodes were purged
	var seasonCount int
	require.NoError(t, db.QueryRow(`SELECT COUNT(*) FROM seasons WHERE title_id = ?`, titleID).Scan(&seasonCount))
	assert.Equal(t, 0, seasonCount)

	var epCount int
	require.NoError(t, db.QueryRow(`SELECT COUNT(*) FROM episodes WHERE season_id = ?`, seasonID).Scan(&epCount))
	assert.Equal(t, 0, epCount)

	// Verify GetByID returns movie without seasons and with series_status cleared
	got, err := repo.GetByID(titleID)
	require.NoError(t, err)
	assert.Equal(t, model.TitleTypeMovie, got.Type)
