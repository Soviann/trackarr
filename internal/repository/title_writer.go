...
func (w *TitleWriter) Create(ctx context.Context, title *model.Title, names []model.TitleName) (int64, error) {
	res, err := w.tx.ExecContext(ctx, `
		INSERT INTO titles (type, is_anime, year, cover_url, imdb_id, anilist_id, tmdb_id, tvdb_id, external_source_id, my_rating, status, series_status, match_status, original_title, match_source, overview, runtime, total_watch_minutes, tmdb_rating, credits, anilist_rating, release_date, next_air_date, next_air_episode, simkl_id, simkl_slug, radarr_id, sonarr_id, arr_ignored)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		title.Type, title.IsAnime, title.Year, title.CoverURL, title.IMDBID, title.AniListID, title.TMDBID, title.TVDBID,
		title.ExternalSourceID, title.MyRating, title.Status, title.SeriesStatus, title.MatchStatus,
...
func (w *TitleWriter) Update(ctx context.Context, id int64, update TitleUpdate) error {
	...
	if update.ExternalSourceID != nil {
		sets = append(sets, `external_source_id = ?`)
		args = append(args, *update.ExternalSourceID)
	}
	...
	if update.Type != nil {
		...
		if *update.Type == model.TitleTypeMovie {
			if _, err := w.tx.ExecContext(ctx, `DELETE FROM seasons WHERE title_id = ?`, id); err != nil {
				return fmt.Errorf("cleanup seasons on movie convert: %w", err)
			}
			sets = append(sets, `series_status = NULL`)
		}
	}
...
func (w *TitleWriter) Merge(ctx context.Context, destID, sourceID int64, seasonOffset int, destAniListID, sourceAniListID int64) error {
		...
		for _, m := range moves {
			...
			default:
				...
				if isAnime {
					...
					// 1. Sync watched state and external_source_id from source episodes to dest episodes
					if _, err := w.tx.ExecContext(ctx, `
						UPDATE episodes
						SET watched = 1,
							external_source_id = COALESCE(episodes.external_source_id, (SELECT src.external_source_id FROM episodes src WHERE src.season_id = ? AND src.episode = episodes.episode))
						WHERE season_id = ?
						  AND episode IN (SELECT src.episode FROM episodes src WHERE src.season_id = ? AND src.watched = 1)
					`, m.id, targetSeasonID, m.id); err != nil {
						return fmt.Errorf("sync watched status for colliding episodes: %w", err)
					}

					// 2. Sync missing episode names/air dates if dest episode is blank
					if _, err := w.tx.ExecContext(ctx, `
						UPDATE episodes
						SET name = COALESCE(name, (SELECT src.name FROM episodes src WHERE src.season_id = ? AND src.episode = episodes.episode)),
							air_date = COALESCE(air_date, (SELECT src.air_date FROM episodes src WHERE src.season_id = ? AND src.episode = episodes.episode))
						WHERE season_id = ?
					`, m.id, m.id, targetSeasonID); err != nil {
						return fmt.Errorf("sync metadata for colliding episodes: %w", err)
					}

					// 3. Re-parent watch_events from source colliding episodes to target episodes
					if _, err := w.tx.ExecContext(ctx, `
						UPDATE watch_events
						SET episode_id = (
							SELECT dest_ep.id
							FROM episodes dest_ep
							JOIN episodes src_ep ON src_ep.episode = dest_ep.episode AND dest_ep.season_id = ?
							WHERE src_ep.id = watch_events.episode_id AND src_ep.season_id = ?
						)
						WHERE episode_id IN (SELECT id FROM episodes WHERE season_id = ?)
					`, targetSeasonID, m.id, m.id); err != nil {
						return fmt.Errorf("re-parent watch events for colliding episodes: %w", err)
					}

					// 4. Move non-colliding episodes from source season to target season
					if _, err := w.tx.ExecContext(ctx, `UPDATE OR IGNORE episodes SET season_id = ? WHERE season_id = ?`, targetSeasonID, m.id); err != nil {
						return fmt.Errorf("merge non-colliding episodes into season %d: %w", targetSeasonID, err)
					}
				}
				// Remaining source episodes are duplicates; their watch_events have been re-parented above.
	...
	// 4. Transfer metadata and external IDs NULL-only.
	if _, err := w.tx.ExecContext(ctx, `UPDATE titles SET
		imdb_id            = COALESCE(titles.imdb_id,            src.imdb_id),
		tmdb_id            = COALESCE(titles.tmdb_id,            src.tmdb_id),
		tvdb_id            = COALESCE(titles.tvdb_id,            src.tvdb_id),
		anilist_id         = COALESCE(titles.anilist_id,         src.anilist_id),
		external_source_id = COALESCE(titles.external_source_id, src.external_source_id),
		sonarr_id          = COALESCE(titles.sonarr_id,          src.sonarr_id),
		radarr_id          = COALESCE(titles.radarr_id,          src.radarr_id),
		cover_url          = COALESCE(titles.cover_url,          src.cover_url),
		accent_hex         = COALESCE(titles.accent_hex,         src.accent_hex),
		overview           = COALESCE(titles.overview,           src.overview),
	...
	// 4d. Transfer genres (INSERT OR IGNORE into title_genres)
	if _, err := w.tx.ExecContext(ctx, `
		INSERT OR IGNORE INTO title_genres (title_id, genre)
		SELECT ?, genre FROM title_genres WHERE title_id = ?
	`, destID, sourceID); err != nil {
		return fmt.Errorf("transfer genres: %w", err)
	}

	// 4e. Clean up pending tasks in task_queue for sourceID
	if _, err := w.tx.ExecContext(ctx, `DELETE FROM task_queue WHERE dedup_key = 'arr_push_' || ?`, sourceID); err != nil {
		return fmt.Errorf("clean up source arr_push tasks: %w", err)
	}

	// 4f. Recalculate total_watch_minutes from watched episodes and runtime
	if _, err := w.tx.ExecContext(ctx, `
		UPDATE titles SET
			total_watch_minutes = (
		...
		WHERE id = ?
	`, destID, destID); err != nil {
		return fmt.Errorf("recalculate total watch minutes: %w", err)
	}
