...
func (r *TitleRepository) searchTitles(searchTerm string, filter TitleFilter) ([]model.Title, error) {
	...
	if useFTS {
		...
		query = `SELECT ` + titleSelectCols + `, tn.name, tn.language FROM titles t JOIN title_names tn ON tn.title_id = t.id JOIN title_names_fts fts ON fts.rowid = tn.id`
		conditions = append(conditions, `title_names_fts MATCH ?`)
		args = append(args, ftsQuery)
	} else {
		query = `SELECT ` + titleSelectCols + `, tn.name, tn.language FROM titles t JOIN title_names tn ON tn.title_id = t.id`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.ID, &t.Type, &t.IsAnime, &t.Year, &t.CoverURL, &t.IMDBID, &t.AniListID, &t.TMDBID, &t.TVDBID,
			...
			&t.ReleaseDate, &t.NextAirDate, &t.NextAirEpisode, &lastWatchedAtStr, &t.AccentHex, &t.CreatedAt, &t.UpdatedAt, &t.CaughtUp,
...
func (r *TitleRepository) fuzzySearch(search string, seen map[int64]bool, filter TitleFilter) ([]model.Title, error) {
	...
	query = `SELECT ` + titleSelectCols + ` FROM titles t WHERE t.id IN (` + placeholders + `)`
	var args []any
