...
func TestTitleService_CreateFromPlex_DuplicateDetection(t *testing.T) {
	...
	// Verify the existing title was updated with external source key
	title, _ := titleRepo.GetByID(existingID)
	require.NotNil(t, title.ExternalSourceID)
	assert.Equal(t, "plex-123", *title.ExternalSourceID)
}

func TestTitleService_CreateFromScrobble_LocalNameFallback(t *testing.T) {
	db, _, err := database.Open(":memory:")
	require.NoError(t, err)
	require.NoError(t, database.Migrate(db))
	defer db.Close()
	...
	// Pre-existing title with English and French names, NO rating key
	existingID := testutil.CreateTitle(t, db, &model.Title{
		Type:        model.TitleTypeSeries,
		Year:        1994,
	...
	// Pipeline is nil to guarantee no external network/API call is made
	svc := service.NewTitleService(db, titleRepo, taskRepo, nil)

	ratingKey := "jellyfin-uuid-999"
	tx, err := db.Begin()
	require.NoError(t, err)
	defer tx.Rollback() //nolint:errcheck

	matchedID, err := svc.CreateFromScrobble(
		context.Background(),
		tx,
		"Wild C.A.T.S: le commando galactique",
		1994,
		service.ExternalIDs{},
		model.TitleTypeSeries,
		ratingKey,
		nil,
		model.TitleStatusWatching,
	)
	require.NoError(t, err)
	require.NoError(t, tx.Commit())

	// Returned the existing ID matching the French alias
	assert.Equal(t, existingID, matchedID)

	// Existing record has its external source key updated
	title, err := titleRepo.GetByID(existingID)
	require.NoError(t, err)
	require.NotNil(t, title.ExternalSourceID)
	assert.Equal(t, "jellyfin-uuid-999", *title.ExternalSourceID)
}
...
func TestTitleService_Rematch_ClearsCoverURL(t *testing.T) {
	db, _, err := database.Open(":memory:")
	require.NoError(t, err)
	require.NoError(t, database.Migrate(db))
	defer db.Close()
