...
func isNilAI(ai AIProvider) bool {
	if ai == nil {
		return true
	}
	if g, ok := ai.(*GeminiClient); ok && g == nil {
		return true
	}
	return false
}

func NewPipeline(tmdb *TMDBClient, anilist *AniListClient, gemini AIProvider, crossDB *CrossRefDB, dataDir string) *Pipeline {
	ai := gemini
	if isNilAI(gemini) {
		ai = nil
	}
...
// SetTMDB injects the TMDB client into the pipeline.
func (p *Pipeline) SetTMDB(tmdb *TMDBClient) { p.tmdb = tmdb }

// SetTVDB injects the TVDB client into the pipeline.
func (p *Pipeline) SetTVDB(tvdb *TVDBClient) { p.tvdb = tvdb }

// SetGemini injects the Gemini or AI client into the pipeline.
func (p *Pipeline) SetGemini(gemini AIProvider) {
	if isNilAI(gemini) {
		p.gemini = nil
	} else {
		p.gemini = gemini
...
// SetAIProvider injects an AIProvider into the pipeline.
func (p *Pipeline) SetAIProvider(ai AIProvider) {
	if isNilAI(ai) {
		p.gemini = nil
	} else {
		p.gemini = ai
...
// SetAniList injects the AniList client into the pipeline.
func (p *Pipeline) SetAniList(anilist *AniListClient) { p.anilist = anilist }
...
// Gemini returns the underlying AI provider.
func (p *Pipeline) Gemini() AIProvider { return p.gemini }

// AI returns the underlying AI provider.
func (p *Pipeline) AI() AIProvider { return p.gemini }

// AniList returns the underlying AniList client.
func (p *Pipeline) AniList() *AniListClient { return p.anilist }
