...
func TestSimklImport_DuplicateSkip(t *testing.T) {
	...
	backup := &service.SimklBackup{
		Movies: []service.SimklItem{{
			...
			Year:   2024,
			Status: "completed",
			IDs:    service.SimklIDs{IMDB: "tt1234567"},
		}},
	}
...
func TestSimklImport_ShowWithEpisodes(t *testing.T) {
	...
	backup := &service.SimklBackup{
		Shows: []service.SimklItem{{
			...
			Year:   2008,
			Status: "completed",
			IDs:    service.SimklIDs{IMDB: "tt0903747", TMDB: 1396},
...
func TestSimklImport_AnimeMovie(t *testing.T) {
	...
	backup := &service.SimklBackup{
		Anime: []service.SimklItem{{
			...
			Status:    "completed",
			AnimeType: "movie",
			IDs:       service.SimklIDs{AniList: 21519},
		}},
	}
...
func TestSimklImport_StatusMapping(t *testing.T) {
	...
	backup := &service.SimklBackup{
		Movies: []service.SimklItem{
			{Title: "A", Year: 2024, Status: "plantowatch", IDs: service.SimklIDs{IMDB: "tt0000001"}},
			{Title: "B", Year: 2024, Status: "notinteresting", IDs: service.SimklIDs{IMDB: "tt0000002"}},
			{Title: "C", Year: 2024, Status: "hold", IDs: service.SimklIDs{IMDB: "tt0000003"}},
		},
	}
