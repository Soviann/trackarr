-- Retire les nouveaux task_type du CHECK — on supprime d'abord toute ligne
-- qui utiliserait une valeur désormais interdite pour que la contrainte passe.
DELETE FROM task_queue WHERE task_type IN ('anilist_push_season', 'anilist_push_movie');

CREATE TABLE task_queue_old (
...
INSERT INTO task_queue_old (id, task_type, payload, status, attempts, max_attempts, day, last_error, run_at, created_at, updated_at, dedup_key)
