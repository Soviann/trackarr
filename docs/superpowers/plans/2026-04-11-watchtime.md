...
- [ ] **Step 1: Add `TotalWatchMinutes` to the repository Title struct in `title.go`**
...
- [ ] **Step 2: Add field to INSERT in `title.go`**
...
- [ ] **Step 3: Add field to SELECT scan in `title.go`**
...
- [ ] **Step 4: Add field to UPDATE in `title.go`**
...
- [ ] **Step 5: Add field to `title_search.go` SELECT + scan**
...
- [ ] **Step 3: Implement `CountByTitleID`**
...
- [ ] **Step 1: Locate `MarkWatched`, `ToggleWatched`, and `BatchMarkWatched` in `library.go`**
...
- [ ] **Step 2: Add helper to safely get runtime**
...
- [ ] **Step 3: In the "mark as watched" path, increment total_watch_minutes**
...
- [ ] **Step 4: In the "mark as unwatched" path, decrement total_watch_minutes**
...
- [ ] **Step 5: In `BatchMarkWatched`, apply the same increment per episode marked**
...
- [ ] **Step 1: In `simkl.go`, after creating each watch event, add runtime to title**
...
- [ ] **Step 2: In `webhook.go`, after recording a Plex scrobble watch event, increment**
...
- [ ] **Step 1: Add `total_watch_minutes` to the Title type in `types.ts`**
...
- [ ] **Step 2: Add a format helper in `frontend/src/utils.ts`**
...
- [ ] **Step 4: Add the CSS class in `TitleDetail.module.css`**
...
- [ ] **Step 1: Add `total_watch_minutes` to the models section and `CountByTitleID` to repositories**
