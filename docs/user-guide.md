...
**Filtre par statut de série** : quand le filtre **Type** est réglé sur **Series**, une section supplémentaire apparaît pour filtrer selon l'avancement de la série — All, Returning (en cours), Ended (terminée), Cancelled (annulée), Not started (pas encore diffusée). « Not started » regroupe toutes les séries annoncées mais dont aucun épisode n'est encore sorti. Le statut est déterminé automatiquement lors du rafraîchissement des métadonnées ; pour les titres déjà en bibliothèque et sans statut renseigné, lancer une fois **Admin → Refresh All** pour le récupérer.
