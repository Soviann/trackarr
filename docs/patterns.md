...
## Status: T33+genre-filter complete
...
`internal/repository/` — All repos use `database.DBTX` interface (works with `*sql.DB` and `*sql.Tx`). TitleRepository (PaginatedResult, TitleFilter with Limit/Offset/UpToDate/WatchingBehind/SeriesStatus/Sort/Order/Genres/GenreOp), SeasonRepository, EpisodeRepository, WatchEventRepository, SettingRepository, StatsRepository, GenreRepository (`ListWithCounts`, `ReplaceForTitle`). All DB queries live here. Title search in `title_search.go`. `List()` returns paginated light response (no episodes, season counters + next_episode). `ListAll()` returns full data for background jobs. `GetByID()` returns full detail with episodes. Genres stored in `title_genres` join table (migration 016), loaded separately to respect MaxOpenConns=1.
