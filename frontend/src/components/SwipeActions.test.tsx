...
function mockActionsWidth(container: HTMLElement, width = 160) {
...
describe('SwipeActions', () => {
  ...
  beforeEach(() => {
    ...
    onFix = vi.fn()
    actions = [
      { icon: '✓', color: 'green', label: 'Confirm', onAction: onConfirm },
      { icon: '✎', color: 'orange', label: 'Fix', onAction: onFix },
    ]
  ...
  it('tapping a specific action calls that action', async () => {
    ...
    const fixBtn = within(container).getByRole('button', { name: 'Fix', hidden: true })
    await act(async () => {
      fixBtn.click()
    })
