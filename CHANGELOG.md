...
- Bandeau d'information AniList par saison sur la fiche détail d'un anime : la saison active affiche désormais son score communautaire AniList et un lien direct vers l'entrée correspondante, entre la ligne de progression et la grille d'épisodes. Conséquence côté barre d'actions : pour les séries multi-saisons l'onglet AniList du titre est masqué (chaque saison a son propre lien là où il pointe vers la bonne entrée), tandis que les films et les séries à une seule saison gardent l'onglet — on ne pointe plus toute une série multi-saisons vers l'entrée d'une seule saison.
- Correction manuelle d'un mapping AniList par saison : le bouton crayon ✎ du bandeau (ou « Link entry » sur une saison non cartographiée) ouvre désormais le panneau de rematch préchargé avec l'ID AniList existant — sauvegarder appelle `PUT /api/titles/{id}/seasons/{sid}/anilist` et déclenche un push immédiat vers AniList, retirer le mapping appelle `DELETE` sur la même route. Avant ce changement il n'existait aucun chemin UI pour corriger un mauvais mapping de saison sans repasser par le flux complet de rematch du titre.
- Bannière de reconnexion AniList dans les paramètres : un push qui se prend un 401 lève désormais le drapeau `anilist_token_invalid` côté serveur, et la page Paramètres affiche « AniList connection expired. Rating & status sync is paused » avec un bouton Reconnect — finir le flux OAuth efface le drapeau et relance la synchronisation. Avant ce changement, un token expiré entraînait des dizaines de pushs silencieusement perdus à chaque visionnage sans aucun retour utilisateur.
- Note par saison retirée : la colonne `seasons.my_rating` et son endpoint `PATCH /api/titles/:id/seasons/:sid` n'existent plus — la note du titre est désormais la seule source de vérité pour la synchronisation AniList et pour l'affichage. Aucun chemin UI n'envoyait de note par saison ; le code a été supprimé pour éviter qu'on en réintroduise une par accident à l'avenir.
- Bouton « Save & sync AniList » retiré du prompt de notation : il n'avait jamais été câblé (`onSaveAndAnilist` n'était passée nulle part) et la synchronisation AniList est désormais automatique. Le prompt ne montre plus que « Save » — tout pousse vers AniList sans intervention.
